...
func movemental():
	if !isDying:
		...
		moveTween.set_parallel(true).tween_property($Model, "global_rotation", $Model.global_rotation + anglature, speed).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_CIRC)
